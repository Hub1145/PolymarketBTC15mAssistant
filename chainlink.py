...
class ChainlinkFetcher:
    ...
    async def fetch_chainlink_btc_usd(self) -> Dict:
        ...
        for rpc in rpcs:
            try:
                w3 = Web3(Web3.HTTPProvider(rpc, request_kwargs={'timeout': 1.5}))
                contract = w3.eth.contract(address=Web3.to_checksum_address(aggregator_address), abi=AGGREGATOR_ABI)

                if self.cached_decimals is None:
                    self.cached_decimals = contract.functions.decimals().call()
